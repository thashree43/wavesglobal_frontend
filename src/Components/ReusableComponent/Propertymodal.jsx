...
const PropertyModal = ({ isOpen, onClose, onSubmit, editingProperty, neighborhoods, propertyTypes, loading }) => {
  ...
  useEffect(() => {
    if (editingProperty) {
      // Ensure images have proper IDs when editing
  ...
  const removeImage = (imageId) => {
    console.log('Removing image with ID:', imageId); // Debug log
    console.log('Current images:', formData.images); // Debug log
    
    setFormData(prev => {
      const filteredImages = prev.images.filter(img => {
        console.log('Comparing:', img.id, 'with', imageId, 'Equal:', img.id === imageId); // Debug log
        return img.id !== imageId;
      });
      
      console.log('Images after filtering:', filteredImages); // Debug log
...
export default PropertyModal;text-lg font-semibold text-gray-900 mb-4">Basic Information</h3>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div className="lg:col-span-2">
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Property Title *
